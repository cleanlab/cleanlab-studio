...
from cleanlab_studio.internal.util import DatasetSourceType, init_dataset_source, as_numpy_type
...
class Studio:
    ...
    def apply_corrections(self, cleanset_id: str, dataset):
        if pyspark_exists and isinstance(dataset, pyspark.sql.DataFrame):
            ...
            joined_ds = dataset.join(cl_cols, on=id_col)
            joined_ds["__cleanlab_final_label"] = cl_cols[label_column].where(cl_cols[label_column].notna(), dataset[label_column])
