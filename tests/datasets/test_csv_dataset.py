...
@pytest.mark.parametrize(("csv_filepath", "csv_df"), (
    (csv_filepath, csv_filepath_to_dataframe(csv_filepath))
    for csv_filepath in CSV_DATASETS
))
def test_csv_dataset_from_filepath_matches_expected(csv_filepath: pathlib.Path, csv_df: pd.DataFrame):
...
def test_csv_dataset_from_fileobj_matches_expected(csv_filepath: pathlib.Path, csv_df: pd.DataFrame):
