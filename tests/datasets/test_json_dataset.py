...
@pytest.mark.parametrize(("json_filepath", "json_df"), (
    (json_filepath, json_filepath_to_dataframe(json_filepath))
    for json_filepath in json_DATASETS
))
def test_json_dataset_from_filepath_matches_expected(json_filepath: pathlib.Path, json_df: pd.DataFrame):
...
def test_json_dataset_from_fileobj_matches_expected(json_filepath: pathlib.Path, json_df: pd.DataFrame):
